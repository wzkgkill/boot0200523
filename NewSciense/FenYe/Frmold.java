 ...
 * 老商品添加主页面
 */
public class Frmold extends PagerFrame {
    ...
    ///实例化DAO类
    RepertoryFenYeDAO dao = new RepertoryFenYeDAO();
    ...
    public Frmold(FrmProcure2 param) {
        this();
        this.parentA = param;
    }
    ...
    public Frmold() {
        ...
        //编号文本框只能输入数字
        txtBianHao.addKeyListener(new KeyAdapter() {
            int keycode = 0;
        ...
        panelFenYe = new JPanel();
        panel_10.add(panelFenYe);
        //初始化分页控件************
        //每页数量
        int pageNo = 7;
        panelFenYe.setLayout(new BoxLayout(panelFenYe, BoxLayout.X_AXIS));
        pager = new PagerUtils(pageNo, Frmold.this);

        //重写PagerFrame的方法showPager。完成分页数据展示
        showPager(pager);
        panelFenYe.add(pager);
     ...
     */
    @Override
    public void showPager(PagerUtils pager) {
        RepertoryFenYeDAO dao = new RepertoryFenYeDAO();
        //获取查询条件****
        ...
        //传递参数，进行查询
        List<Repertory> list = dao.selectByPager(pager, rsid, name, min, max);
        DefaultTableModel model = (DefaultTableModel) table_1.getModel();
        model.setRowCount(0);
